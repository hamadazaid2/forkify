...
class RecipeView extends View {
  ...
  _generateFirstTimeLoadedPage() {
    this._parentElement.textContent = 'Page is loaded for the first time ;)'
  }
  ...
  _generateMarkup() {
    return `
        ...
        <div class="recipe__ingredients">
          ...
          ${this._data.ingredients.map(ing => this._generateMarkpIngredient(ing)).join('')}
          </ul>
        </div>
  ...
  _generateMarkpIngredient(ing) {
    return `
            <li class="recipe__ingredient">
              <svg class="recipe__icon">
                <use href="${icons}#icon-check"></use>
              </svg>
              <div class="recipe__quantity">${ing.quantity ? new Fraction(ing.quantity) : '0'}</div>
